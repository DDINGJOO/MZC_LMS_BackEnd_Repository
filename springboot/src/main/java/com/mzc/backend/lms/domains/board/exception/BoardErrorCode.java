 ...
 * Board 도메인 에러 코드
 * <p>
 * 게시판, 게시글, 댓글, 첨부파일, 과제 관련 에러 코드를 정의합니다.
 * </p>
 *
 * @see DomainErrorCode
 */
@Getter
public enum BoardErrorCode implements DomainErrorCode {
    ...
    EMPTY_FILE("FILE_005", "파일이 비어있습니다", HttpStatus.BAD_REQUEST),
    INVALID_FILENAME("FILE_006", "파일명이 없습니다", HttpStatus.BAD_REQUEST),
    FILE_SAVE_FAILED("FILE_007", "파일 저장에 실패했습니다", HttpStatus.INTERNAL_SERVER_ERROR),
    FILE_NOT_FOUND("FILE_008", "파일이 존재하지 않습니다", HttpStatus.NOT_FOUND),

    // Hashtag 관련 (HASHTAG_0XX)
    INVALID_HASHTAG("HASHTAG_001", "해시태그 이름은 필수입니다", HttpStatus.BAD_REQUEST),
    ...
    public String toString() {
        return String.format("BoardErrorCode{domain='%s', code='%s', message='%s', status=%s}",
